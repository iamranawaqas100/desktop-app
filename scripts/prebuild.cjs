  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (i === retries - 1) {
        ...
        console.log('   This is usually okay, electron-builder will handle it.\n');
        return false;
      }
      // Wait before retry
      const delay = (i + 1) * 1000;
      console.log(`   Retrying in ${delay}ms...`);
      execSync(`timeout /t ${delay / 1000} /nobreak > nul 2>&1 || sleep ${delay / 1000}`, { stdio: 'ignore' });
    }
  }
  return false;
}
...
if (fs.existsSync(distRenderer)) {
  console.log('🧹 Cleaning previous renderer build...\n');
  safeRemove(distRenderer);
}
