...
const checks = [
  ...
  },
  {
    ...
    check: () => {
      const exists = fs.existsSync(path.resolve(__dirname, '../assets/icon.ico'));
  ...
  },
  {
    ...
    check: () => {
      const exists = fs.existsSync(path.resolve(__dirname, '../../../packages/ui/src'));
      return { pass: exists, message: exists ? '✓' : '✗' };
    }
  },
  {
  ...
  },
  {
    ...
    check: () => {
      try {
        ...
        const hasEB = pkg.devDependencies && pkg.devDependencies['electron-builder'];
