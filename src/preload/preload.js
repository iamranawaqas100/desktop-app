...
contextBridge.exposeInMainWorld("electronAPI", {
  // Data operations - MongoDB with collection context
  getExtractedData: (context) =>
    ipcRenderer.invoke("get-extracted-data", context),
  saveExtractedData: (data, context) =>
    ipcRenderer.invoke("save-extracted-data", data, context),
  updateExtractedData: (id, data) =>
    ipcRenderer.invoke("update-extracted-data", id, data),
  deleteExtractedData: (id) => ipcRenderer.invoke("delete-extracted-data", id),
  verifyAIItem: (aiItemId, collectorId, updates) =>
    ipcRenderer.invoke("verify-ai-item", aiItemId, collectorId, updates),
  exportData: (format) => ipcRenderer.invoke("export-data", format),

  // Menu event listeners
  onMenuNewExtraction: (callback) =>
    ipcRenderer.on("menu-new-extraction", callback),
  ...
  onMenuFindSimilar: (callback) =>
    ipcRenderer.on("menu-find-similar", callback),

  // Browser view communication
  sendToBrowser: (channel, data) =>
    ipcRenderer.send("send-to-browser", channel, data),
  ...
  onAICollectionCallback: (callback) =>
    ipcRenderer.on("ai-collection-callback", (event, data) => callback(data)),

  // View AI Menu callback
  onViewAIMenu: (callback) =>
    ipcRenderer.on("view-ai-menu", (event, data) => callback(data)),

  // Manual collection handlers
  saveManualMenuItems: (data) =>
    ipcRenderer.invoke("save-manual-menu-items", data),
  getCollectionContext: (data) =>
    ipcRenderer.invoke("get-collection-context", data),
...
ipcRenderer.on("send-to-browser", () => {
