  ...
  switch (type) {
    case "image": {
      ...
      if (childImg) {
        return (
          childImg.src ||
          childImg.getAttribute("data-src") ||
          childImg.getAttribute("data-lazy")
        );
      }
      return "";
    }
    ...
    case "price": {
      ...
      const fromPricePattern =
        /From\s*(?:Rs\.?\s*|PKR\s*|₹\s*|[$€£¥₽₩￥]\s*)[,\d]+(?:\.\d{1,2})?/gi;
      const fromMatch = priceText.match(fromPricePattern);
      if (fromMatch) {
        return fromMatch[0].trim();
      }

      const numberCurrencyPattern =
        /[,\d]+(?:\.\d{1,2})?\s*(?:Rs\.?|PKR|₹|[$€£¥₽₩￥])/gi;
...
function findSimilar() {
  ...
  const templateFields = Object.keys(currentTemplate).filter(
  ...
  containers.forEach((container) => {
    ...
    templateFields.forEach((field) => {
      ...
      const similarElement = findSimilarElementInContainer(
        container,
        templateInfo
      );
  ...
  while (container && level < 5) {
    ...
    if (children.length >= 3) {
      ...
      if (
        ...
        container.className.includes("row") ||
...
function findSimilarElementInContainer(container, templateInfo) {
  try {
    const exactMatch = container.querySelector(
      templateInfo.selector.split(" ").pop()
    );
    if (exactMatch) return exactMatch;
  } catch (e) {
    // Ignore errors when finding exact match
  }
